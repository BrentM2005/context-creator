...
export default function DownloadPage() {
  ...
  if (loading) return <div className="text-center py-20 text-lg font-medium text-text-muted animate-pulse">{t('dl_loading')}</div>;
  if (error || releases.length === 0) return <div className="text-center py-20 text-lg text-red-500">{t('dl_error')}</div>;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-6xl mx-auto px-6 py-16">
      <div className="text-center mb-16 flex flex-col items-center">
        <h1 className="text-4xl font-extrabold mb-6 text-text-dark">{t('dl_title')}</h1>
        <div className="flex items-center gap-3 bg-white border border-border-dark py-2 px-4 rounded-lg shadow-sm">
          ...
          <select id="version-select" value={activeVersionId ?? ''} onChange={(e) => setSelectedVersionId(Number(e.target.value))} className="bg-transparent text-text-dark font-bold text-lg outline-none cursor-pointer focus:ring-0">
            {releases.map(r => (
              <option key={r.id} value={r.id}>{r.tag_name} {r.id === releases[0].id ? '(Latest)' : ''}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
        {cards.map(card => (
          <div key={card.key} className="bg-white border border-border-dark rounded-2xl p-8 flex flex-col hover:border-primary hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 relative overflow-hidden group">
            ...
            <a href={card.file || '#'} onClick={(e) => { if (!card.file) e.preventDefault(); }} className={`flex items-center justify-center py-3 px-4 rounded-xl font-bold transition-all duration-200 shadow-sm ${card.file ? 'bg-primary text-white hover:bg-primary-hover hover:shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200'}`}>
      ...
      {selectedRelease?.body && (
        <div className="mt-16 max-w-4xl mx-auto bg-white border border-border-dark rounded-2xl p-8 shadow-sm">
          ...
          <div className="prose prose-slate max-w-none text-text-muted whitespace-pre-wrap text-sm leading-relaxed" dir="ltr">
            {selectedRelease.body}
          </div>
        </div>
      )}
